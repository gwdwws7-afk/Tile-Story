...
namespace I2.Loc
{
	public partial class LanguageSourceData
	{
        ...
        /// <summary>
        /// 单纯进行输出目标语言的Terms组合的txt
        /// </summary>
        /// <param name="Builder"></param>
        /// <param name="languages"></param>
        public void ExportTargetTermInMulLanguages(string FileName,StringBuilder Builder) {

			Dictionary<string, StringBuilder> dic = new Dictionary<string, StringBuilder>();
			for (int i = 0; i < mLanguages.Count; i++)
			{
				string name = mLanguages[i].Name;
				if (System.Enum.TryParse(name, out Language languageType))
				{
					string key = GetFontName(languageType);
					if (!dic.ContainsKey(key)) { dic[key] = new StringBuilder();
						string result = Builder.ToString();
						dic[key].Append("1234567890");
						dic[key].Append("abcdefghijklmnopqrstuvwxyz");
						dic[key].Append("!&%@$(&%#()_*)_^*(^&*!@+}{|\":?><|[]'\\/.,'");
                    }
					foreach (var termData in mTerms)
					{
						string translation = termData.Languages[i];
						translation = termData.GetTranslation(i, null);
						dic[key].Append(translation);
					}
				}
				else
				{
					Debug.LogError("ParseFault " + name);
				}
			}
			foreach(var result in dic)
            {
				System.IO.File.WriteAllText(FileName.Replace(".csv", "_" + result.Key) + ".txt", result.Value.ToString(), System.Text.Encoding.UTF8);
			}
        }
        public void AppendLanguages(string targetLanguageName,StringBuilder Builder) {
			...
			for (int i = 0; i < mLanguages.Count; i++)
			{
				...
				{
					string key = GetFontName(languageType);
					if (targetLanguageName != key) continue;
					if (!dic.ContainsKey(key)) { dic[key] = new StringBuilder();
						string result = Builder.ToString();
						dic[key].Append("1234567890");
						dic[key].Append("abcdefghijklmnopqrstuvwxyz");
