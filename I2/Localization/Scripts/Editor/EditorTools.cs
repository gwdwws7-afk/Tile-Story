...
{	
  ...
  //           return Reflection_InvokeMethod( typeof( EditorGUI ), "DoTextField", s_RecycledEditor, controlID, position, text, style, null, false, false, false, false ) as string;
		...
		static public object Reflection_InvokeMethod(Type targetType, string methodName, object instance, params object[] p_args)
		{
			BindingFlags flags = BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static;

			MethodInfo mi = targetType
				.GetMethods(flags)
			...
			// static method → instance 必须是 null
			if (mi.IsStatic)
				return mi.Invoke(null, p_args);

			// instance method → instance 不能为 null
			return mi.Invoke(instance, p_args);
		}
		...
		public static string TextField(Rect position, string text, int maxLength, GUIStyle style, int controlID)
		{
			...
			// 创建或获取一个 control id（使得与其它控件的交互保持一致）
			int id = GUIUtility.GetControlID(controlID, FocusType.Keyboard, position);

			EditorGUI.BeginChangeCheck();
			// 使用公开的 EditorGUI.TextField（稳定、兼容）
			string result = EditorGUI.TextField(position, text, style);
			...
			{
				// 变更时你可以在这里执行需要的回调（如果原来 DoTextField 会触发 changed）
				// e.g. EditorUtility.SetDirty(someObject) 或记录 undo
			}
