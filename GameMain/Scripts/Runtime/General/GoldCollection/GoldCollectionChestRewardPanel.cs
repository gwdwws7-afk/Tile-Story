...
public class GoldCollectionChestRewardPanel : RewardPanel
{
    ...
    public override void OnShow(RewardArea rewardArea, Action onShowComplete)
    {
        ...
        string chestAnimName = "purple_open1";
        string chestTopAnimName = "purple_open2";
        chest.Initialize(true);
        chestTop.Initialize(true);
        chest.AnimationState.SetAnimation(0, chestAnimName, false).Complete += t =>
        {
        ...
        if (mapTop != null)
        {
            chestObj.transform.position = mapTop.goldCollectionEntrance.taskRewardOriginalPos;
        }
        chest.gameObject.SetActive(true);
        chestTop.gameObject.SetActive(true);

        chestTop.transform.DOScale(Vector3.one, 0.5f).SetEase(Ease.InOutSine);
        chestObj.transform.DOLocalJump(new Vector3(0, -400f), 700f, 1, 0.65f).SetEase(Ease.InOutQuad).onComplete = () =>
        {
            chest.timeScale = 1;
