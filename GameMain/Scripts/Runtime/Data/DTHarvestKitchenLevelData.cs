...
[Serializable]
public class DTHarvestKitchenLevelData
{
    public List<DTHarvestKitchenLevelDatas> KitchenLevelDatas;

    public DTHarvestKitchenLevelDatas GetKitchenLevelDataByLevel(int level)
    {
...
[Serializable]
public class DTHarvestKitchenLevelDatas
{
	public int ID;
	...
	public string FoodNeedNumber;	//关卡食物必出种类
	...
	private List<int> foodNeedNumberList;
	...
	/// 关卡食物必出种类
	/// </summary>
	public List<int> FoodNeedNumberList
	{
		get
		{
			if (foodNeedNumberList != null && foodNeedNumberList.Count > 0) 
				return foodNeedNumberList;
			foodNeedNumberList = new List<int>();
			var splits = FoodNeedNumber.Split(',');
			foreach (var split in splits)
			{
				foodNeedNumberList.Add(int.Parse(split));
			}
