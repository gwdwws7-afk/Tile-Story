    ...
    Rect[] NSA_iPhone14Pro = new Rect[]
{
    ...
    public void ApplySafeArea(Rect r)
    {
        ...
        if (safeAreas == null || safeAreas.Length == 0) 
        {
            ...
            {
                float width = 1920 * Screen.width / Screen.height;
                float paddingX = (width - 1080) / 2f;
                rectTransform.offsetMin = new Vector2(paddingX, GetBannerAdsHeight());
                rectTransform.offsetMax = new Vector2(-paddingX, 0);

                // GameManager.UI.ShowUIForm("FuzzyBoundary", "GeneralUI");
                // if (!GameManager.DataNode.GetData("ShowFuzzyBoundary", false)) 
                // {
                //     GameManager.DataNode.SetData("ShowFuzzyBoundary", true);
                //     GameManager.UI.ShowUIForm("FuzzyBoundary", "GeneralUI");
                // }
            }
            else
            {
                rectTransform.offsetMin = new Vector2(0, GetBannerAdsHeight());
                rectTransform.offsetMax = new Vector2(0, 0);

                // if (GameManager.DataNode.GetData("ShowFuzzyBoundary", false))
                // {
                //     GameManager.DataNode.SetData("ShowFuzzyBoundary", false);
                //
                //     GameManager.UI.HideUIForm("FuzzyBoundary", "GeneralUI");
                // }
            }
        }
    }

    public static float GetSafeAreaMaxHeight()
    {
        ...
        return Screen.safeArea.y + Screen.safeArea.height;
    }
    ...
    public static float GetScaleChangeByScreenHeight()
    {
        if (Screen.safeArea.width <= 0 || Screen.safeArea.height <= 0) return 1f;
        //长屏手机情况
        if (Screen.height * 1080 > 1920 * Screen.width)
        {
            return Screen.height*1080 / (float)(1920* Screen.width);
        }
        return 1f;
    }
}
