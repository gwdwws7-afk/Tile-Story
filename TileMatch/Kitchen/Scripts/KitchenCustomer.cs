...
public class KitchenCustomer : MonoBehaviour
{
    ...
    public void Init(int id, KitchenCustomerData data, Vector3 moveTarget, Action action = null)
    {
        ...
        {
            needImg[i].sprite = KitchenManager.Instance.GetTileSpriteById(itemId[i]);
        ...
        customerSpine.AnimationState.SetAnimation(0, $"idle{customerId}", true);
        if (customerId == 5 || customerId == 7)
        {
            customerSpine.transform.localPosition = Vector3.up * -297f;
        }
        else
        {
            customerSpine.transform.localPosition = Vector3.up * -327f;
        }
