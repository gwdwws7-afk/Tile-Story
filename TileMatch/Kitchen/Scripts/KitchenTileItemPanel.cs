...
public class KitchenTileItemPanel : MonoBehaviour
{
    public int itemsCount = 0;
    // 当前行的前一行，不存在则为空
    public KitchenTileItemPanel upTileItemPanel;
    // 棋子的预设体
    public GameObject KitchenTileItemPrefab;
    public Transform[] positions;
    [SerializeField] private KitchenTileItem[] items = new KitchenTileItem[7];
    private Action<KitchenTileItemPanel> removeAllItemAction;

    // private bool[] isFall = { false, false, false, false, false, false, false };
    public void Init(KitchenTileItemPanel upTileItemPanel, float coinProbabilty, List<int> tileTypeList,
        List<int> tileTypeProbabilityList, List<int> neededFoodsId, Action<KitchenTileItemPanel> recoveryAction,
        Action<KitchenTileItem> itemClickAction, ref List<KitchenTileItem> tileItemPool)
    {
        ...
        for (int i = 0; i < positions.Length; i++)
        {
            KitchenTileItem item = null;
            if (items[i] == null)
            {
                if (tileItemPool.Count == 0) 
                {
                    item = Instantiate(KitchenTileItemPrefab, positions[i].transform).GetComponent<KitchenTileItem>();
                }
                else
                {
                    item = tileItemPool[0];
    ...
    public void RemoveItem(int index)
    {
        if (upTileItemPanel)
        {
            KitchenTileItem item = upTileItemPanel.GetTileItemByIndex(index, positions[index]);
    ...
    public KitchenTileItem GetTileItemByIndex(int index, Transform parent)
    {
        if (itemsCount > 0 && items[index] != null)
        {
            KitchenTileItem temp = items[index];
