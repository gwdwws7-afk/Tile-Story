...
public class KitchenChosenBar : MonoBehaviour
{
	private bool isStartTileMoveAnim = false;

	public Image RedBottomBG;

	[SerializeField] private KitchenGameMenu menu;
	
	private Transform[] TileMatchPositions;
	[SerializeField]
	private Transform[] TileMatchPositions_7;
	
    public OrderDictionary<int, List<KitchenTileItem>> chooseItemDict = new OrderDictionary<int, List<KitchenTileItem>>();
	public Dictionary<int, List<KitchenTileItem>> matchDict = new Dictionary<int, List<KitchenTileItem>>();

	public bool RecordChooseTile(KitchenTileItem tileItem, CallBack<KitchenTileItem> recoveryItem)
    {
	    if (tileItem)
	    {
		    ...
		    if (!chooseItemDict.ContainsKey(tileId))
			    chooseItemDict.Add(tileId, new List<KitchenTileItem>());
            ...
            foreach (var itemKey in chooseItemDict.Keys)
            {
                ...
                {
					List<KitchenTileItem> list = new List<KitchenTileItem>();
					for (int i = 0; i < 3; i++)
					{
    ...
    Dictionary<KitchenTileItem, TileSequence> recordSequenceDict = new Dictionary<KitchenTileItem, TileSequence>();
    private void TilePosMove(int animType, int posIndex, KitchenTileItem tile, CallBack finishAction = null, bool isDestroy = false, bool isHaveAnim = true)
	{
    ...
    /// 接关，清空选择栏中的棋子
    /// </summary>
    public void LevelContinue(CallBack<KitchenTileItem> recoveryItem)
    {
	    foreach (var key in chooseItemDict.Keys)
	    {
