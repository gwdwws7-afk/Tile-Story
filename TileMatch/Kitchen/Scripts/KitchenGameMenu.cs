...
public class KitchenGameMenu : CenterForm, ICustomOnEscapeBtnClicked
{
    ...
    [SerializeField] private GameObject tileItemPrefab;
    ...
    [SerializeField] private KitchenChosenBar chosenBar;
    ...
    private KitchenGamePanelData panelData;
    ...
    [SerializeField] private List<KitchenTileItemPanel> kitchenTileItemPanelPool;
    [SerializeField] private List<KitchenTileItemPanel> noUseKitchenTileItemPanelPool;

    private List<KitchenTileItem> kitchenTileItemPool;
    
    // 当前服务的顾客，最大为2
    private List<KitchenCustomer> currentCustomerList;
    
    public List<KitchenCustomer> KitchenCustomerPool;
    ...
    public override void OnInit(UIGroup uiGroup, Action completeAction = null, object userData = null)
    {
        ...
        DTKitchenTaskDatas currentTaskData = KitchenManager.Instance.GetCurrentTaskDatas();
        ...
        // 初始化关卡数据
        panelData = new KitchenGamePanelData();
        
        if (panelData == null)
        {
            Log.Error("Kitchen:初始化关卡数据失败");
        }
        ...
        // 清空当前餐厅的两个顾客位置
        currentCustomerList = new List<KitchenCustomer>();

        tileMovePanel.localPosition = new Vector3(0, -140f, 0);

        // 每次调用OnRelease后都需要重新申请空间，否则在关闭界面调用OnRelease时会报错
        kitchenTileItemPool = ListPool<KitchenTileItem>.Get();
        kitchenTileItemPanelPool = ListPool<KitchenTileItemPanel>.Get();
        noUseKitchenTileItemPanelPool = ListPool<KitchenTileItemPanel>.Get();
        KitchenCustomerPool = ListPool<KitchenCustomer>.Get();
        ...
        if (kitchenTileItemPanelPool.Count + noUseKitchenTileItemPanelPool.Count < 9)
        {
            for (int i = kitchenTileItemPanelPool.Count + noUseKitchenTileItemPanelPool.Count; i < 9; i++)
            {
                ...
                KitchenTileItemPanel panel = tilePanel.GetComponent<KitchenTileItemPanel>();
    ...
    public override void OnShow(Action<UIForm> showSuccessAction = null, object userData = null)
    {
        ...
        if (currentCustomerList != null) 
        {
            NewCustomerEnter(() =>
            {
                if (!KitchenManager.Instance.IsShowGameGuide)
                {
                    // 顾客进入后开启游戏
                    isStartTileMove = true;
                    KitchenManager.Instance.canClickTile = true;
                    closeBtn.interactable = true;
                }
                else
                {
                    KitchenManager.Instance.IsShowGameGuide = false;
                    ShowGameGuide(0);
                }
            });
        }
    ...
    public override void OnUpdate(float elapseSeconds, float realElapseSeconds)
    {
        if (isStartTileMove)
        {
            ...
            if (time >= 0.2f)
            {
                ...
                {
                    KitchenManager.Instance.toContinueMenuType = 0;
                    GamePause();
                ...
                if (kitchenTileItemPanelPool[0].transform.localPosition.y + tileMovePanel.localPosition.y >= 167f)
                {
                    if (!isPlayCordonAnim)
                    {
                        // 播放Cordon闪烁动画
                        SetCordonAnim(true);
                        if (KitchenManager.Instance.IsShowTopGuide)
                        {
                            KitchenManager.Instance.IsShowTopGuide = false;
                            ShowGameGuide(1);
                        }
                    }
                }
            ...
            if (refreshTilePanelTime >= differValue / speed)
            {
                ...
                // 刷新下方的棋子
                KitchenTileItemPanel panel = null;
                if (noUseKitchenTileItemPanelPool.Count <= 0)
                {
                    GameObject tilePanel = Instantiate(tilePanelPrefab, tileMovePanel);
                    panel = tilePanel.GetComponent<KitchenTileItemPanel>();
                    panel.transform.localPosition = lastPosition;
                    panel.gameObject.SetActive(true);
                    lastPosition.y -= differValue;
                }
        ...
        if (canCloseGuide)
        {
...
#endif
            {
                ...
                if (oldParent != null)
                {
                    ...
                    customerParent.SetSiblingIndex(2);
                }
    ...
    public override void OnRelease()
    {
        ...
        if (KitchenCustomerPool != null)
        {
            ...
            {
                ListPool<KitchenCustomer>.Release(KitchenCustomerPool);
            }
        ...
        if (kitchenTileItemPanelPool != null)
        {
            ...
            {
                ListPool<KitchenTileItemPanel>.Release(kitchenTileItemPanelPool);
            }
        ...
        if (noUseKitchenTileItemPanelPool != null)
        {
            ...
            if (noUseKitchenTileItemPanelPool.Count > 0)
            {
                ListPool<KitchenTileItemPanel>.Release(noUseKitchenTileItemPanelPool);
            }
        ...
        if (kitchenTileItemPool != null)
        {
            ...
            {
                ListPool<KitchenTileItem>.Release(kitchenTileItemPool);
            }
    ...
    public void GamePause(bool isNoFocus = true)
    {
        ...
        // 关闭棋子点击事件
        KitchenManager.Instance.canClickTile = false;
    }
    ...
    public void RecoveryTileItemPanel(KitchenTileItemPanel panel)
    {
    ...
    {
        //玩家主动点击退出按钮时
        KitchenManager.Instance.toContinueMenuType = 1;
        GamePause();
        ShowGameContinue();
    }

    // 点击棋子后的回调
    private void OnTileClick(KitchenTileItem item)
    {
        ...
        if (!gameLose && KitchenManager.Instance.IsShowChooseBarGuide)
        {
            if (chosenBar.GetChooseTotalNum() == 6)
            {
                KitchenManager.Instance.IsShowChooseBarGuide = false;
                ShowGameGuide(2);
            }
        }
    ...
    public bool FoodElimination(KitchenTileItem item, Vector3 initPos)
    {
        // 判断是否是金币，金币直接飞入金币栏
        if (item.itemID == 12)
        {
        ...
        for (int i = 0; i < currentCustomerList.Count; i++)
        {
            ...
            if (needIndex >= 0)
            {
                ...
                GameManager.Task.AddDelayTriggerTask(0.25f, () =>
                {
                    ...
                    flyObj.m_IconImage.sprite = KitchenManager.Instance.GetTileSpriteById(item.itemID);
    ...
    /// <returns>新顾客</returns>
    private KitchenCustomer GetNewCustomer()
    {
        KitchenCustomerData data = panelData.GetCustomerDataByIndex(currentCustomerIndex);
        if (data == null) return null;
        
        KitchenCustomer customer = null; 
        if (KitchenCustomerPool.Count <= 0)
        {
            // 对象池中对象数量不足，开始创建
            GameObject obj = Instantiate(customerPrefab, customerParent);
            customer = obj.GetComponent<KitchenCustomer>();
        ...
        {
            customer = KitchenCustomerPool[0];
            KitchenCustomerPool.RemoveAt(0);
        }

        int id = customerIdList[Random.Range(0, customerIdList.Count)];
    ...
    public void InitKitchenCustomerPool()
    {
        ...
        {
            GameObject obj = Instantiate(customerPrefab, customerParent);
            var customer = obj.GetComponent<KitchenCustomer>();
            customer.SetTimeoutEvent(()=>
            {
                KitchenManager.Instance.toContinueMenuType = 0;
                GamePause();
    ...
    private void GameWin()
    {
        ...
        // 记录胜利数据
        KitchenManager.Instance.ActivityLevelWin(currentPraiseNum);
    ...
    {
        GameManager.UI.ShowUIForm("KitchenGameContinueMenu", form =>
        {
            var uiform = form as KitchenGameContinueMenu;
