...
public class KitchenTileItem : MonoBehaviour
{
    // 棋子的类型ID
    public int itemID = 0;
    // 棋子在父节点中的编号
    ...
    private Action<KitchenTileItem> clickAction2;
    public bool isSelected;
    public bool isClicked;

    private List<int> delayList;
    
    public void Init(int index, int id, Transform parent, Action<int> action, Action<KitchenTileItem> clickTileAction)
    {
        itemID = id;
        itemIndex = index;
        btn.enabled = true;
        tileBg.gameObject.SetActive(true);
        icon.sprite = KitchenManager.Instance.GetTileSpriteById(itemID);
        icon.SetNativeSize();
    ...
    {
        if (!KitchenManager.Instance.canClickTile) return;
    ...
    public void DestroyTile(bool isPlayAudio = true, string destroySound = null, CallBack<KitchenTileItem> finishCallBack = null)
    {
        try
        {
            if (isPlayAudio)
