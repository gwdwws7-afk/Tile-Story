...
namespace Merge
{
    public class MergeChristmasMenu : UIForm
    {
        ...
        public ChristmasRewardBubble[] m_Bubbles;
        ...
        public GameObject m_ChristmasTitle;
        ...
        public override void OnInit(UIGroup uiGroup, Action completeAction = null, object userData = null)
        {
            ...
            GameManager.Event.Subscribe(CommonEventArgs.EventId, OnChristmasBubbleGetRewardRVTimeUpdate);
            ...
            RefreshBubble(false);
            RefreshChristmasBubbleGetRewardRVTime();
        }
        ...
        public override void OnShow(Action<UIForm> showSuccessAction = null, object userData = null)
        {
            ...
            if (m_CanDecorate && PlayerPrefs.GetInt("HasShowedChristmasDecorateGuide", 0) == 0) 
            {
                PlayerPrefs.SetInt("HasShowedChristmasDecorateGuide", 1);
        ...
        {
            int stage = MergeManager.PlayerData.GetChristmasDecorationStage();
            ...
            if (m_CanDecorate)
            {
                string christmasName = "Christmas_" + (stage + 1).ToString();
                if (decorateAssetName != christmasName)
                {
                    decorateAssetName = christmasName;
                    spriteHandle = UnityUtility.LoadAssetAsync<Sprite>(UnityUtility.GetAltasSpriteName(christmasName, "MergePropAtlas_Christmas"), sp =>
                    {
            ...
            if (!string.IsNullOrEmpty(animName))
            {
                if (showAnim)
                {
                    m_TreeDecorationAnim.AnimationState.SetAnimation(0, animName, false).Complete += t =>
                    {
                        if (stage == 6)
                        {
                            m_ChristmasTitle.transform.localScale = Vector3.zero;
                            m_ChristmasTitle.gameObject.SetActive(true);
                            m_ChristmasTitle.transform.DOScale(1.1f, 0.2f).onComplete = () =>
                            {
                                m_ChristmasTitle.transform.DOScale(1f, 0.2f);
                            };
                        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        m_ChristmasTitle.gameObject.SetActive(false);
                    }
        ...
        public void RefreshBubble(bool allCanGet)
        {
            ...
            for (int i = 0; i < m_Bubbles.Length; i++)
            {
                if (i < bubbleMaxNum) 
                {
                    if (MergeManager.PlayerData.GetChristmasBubbleRewardId(i) > 0)
                    {
                        if(allCanGet)
        ...
        public void RefreshChristmasBubbleGetRewardRVTime()
        {
            getRewardRVTime = MergeManager.PlayerData.GetChristmasBubbleGetRewardRVTime();
            m_IsStartRVTime = getRewardRVTime != Constant.GameConfig.DateTimeMin;
            m_CurrentRVTimeInterval = 0;
        }

        public void OnChristmasBubbleGetRewardRVTimeUpdate(object sender, GameEventArgs e)
        {
            var ne = e as CommonEventArgs;
            if (ne == null) return;
            if (ne.Type == CommonEventType.ChristmasBubbleGetRewardRVTime)
            {
                RefreshChristmasBubbleGetRewardRVTime();
            }
        }
        ...
        private void OnCloseButtonClick()
        {
            ...
            GameManager.Process.EndProcess(ProcessType.AutoShowChristmasProcess);
        }
        ...
        {
            int stage = MergeManager.PlayerData.GetChristmasDecorationStage() + 1;
            if (stage > 6)
                return;
            ...
            MergeManager.PlayerData.SetChristmasDecorationStage(stage);
            if (stage == 6)
            {
            ...
            GameManager.Event.Fire(this, ChristmasDecorateEndEventArgs.Create());
