namespace Merge
{
    //Double tap the exclusive fish to get it!
    public sealed class Guide_TapMaxReward : Guide
    {
        private PropLogic m_TargetProp;

        public Guide_TapMaxReward(MergeGuideMenu guideMenu) : base(guideMenu)
        {
        }

        public override GuideTriggerType GuideType => GuideTriggerType.Guide_TapMaxReward;

        public override int GuidePriority => 3;
        ...
        public override void OnTriggerGuide()
        {
            ...
            if (mainBoard != null && m_TargetProp != null && m_TargetProp.Square != null)
            {
                ...
                if (MergeManager.Instance.Theme == MergeTheme.DigTreasure) 
                    m_GuideMenu.Show("DigMerge.GuideDialog7", m_TargetProp.Square.transform.position, 0.3f);
                else
                    m_GuideMenu.Show("Merge.Guide8", m_TargetProp.Square.transform.position, 0.3f);
                m_GuideMenu.ShowFingerAnim(m_TargetProp.Square.transform.position, m_TargetProp.Square.transform.position);
                m_GuideMenu.ShowRaycastMask(m_TargetProp.Square.transform.position, 40, 40, 0.8f);
            }
            else
            {
