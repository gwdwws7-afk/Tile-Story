...
namespace Merge
{
    //This is at its max level!Double tap to claim rewards!
    public sealed class Guide_DateMerge_MaxLevel : Guide
    {
        private PropLogic m_TargetProp;

        public Guide_DateMerge_MaxLevel(MergeGuideMenu guideMenu) :base(guideMenu)
        {
        }

        public override GuideTriggerType GuideType => GuideTriggerType.Guide_MaxLevel;

        public override int GuidePriority => 2;
        ...
        public override void OnTriggerGuide()
        {
            ...
            if (mainBoard != null && m_TargetProp != null && m_TargetProp.Square != null) 
            {
                ...
                m_GuideMenu.Show("Merge.Guide6", m_TargetProp.Square.transform.position, 0.35f);
