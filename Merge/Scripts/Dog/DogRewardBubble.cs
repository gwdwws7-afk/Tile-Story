...
namespace Merge
{
    public class DogRewardBubble : MonoBehaviour
    {
        ...
        private int m_Index;
        private MergeDogMenu m_Menu;
        private bool m_IsShowing;
        private DRDogBubbleReward m_RewardData;
        private string m_RewardSpriteKey;
        private AsyncOperationHandle m_AsyncHandle;

        public bool IsShowing => m_IsShowing;

        public void Initialize(int index, MergeDogMenu menu)
        {
            m_Index = index;
            m_Menu = menu;
            m_IsShowing = false;
            ...
            if (MergeManager.PlayerData.GetDogBubbleRewardId(m_Index) > 0) 
                Refresh(false);
        }
        ...
        {
            int rewardId = MergeManager.PlayerData.GetDogBubbleRewardId(m_Index);
            if (rewardId <= 0)
            {
                rewardId = MergeManager.Instance.GetRandomBubbleRewardId();
                MergeManager.PlayerData.SetDogBubbleRewardId(m_Index, rewardId);
                DRDogBubbleReward rewardData = MergeManager.Instance.GetDogBubbleRewardData(rewardId);

                if (!allCanGet)
                    MergeManager.PlayerData.SetDogBubbleGetRewardTime(m_Index, DateTime.Now.AddMinutes(rewardData.CD));
            }

            m_RewardData = MergeManager.Instance.GetDogBubbleRewardData(rewardId);

            if (allCanGet)
                MergeManager.PlayerData.ClearDogBubbleGetRewardTime(m_Index);
        ...
        {
            m_Timer.OnReset();
            DateTime getRewardTime = MergeManager.PlayerData.GetDogBubbleGetRewardTime(m_Index);
        ...
        public void GetDogBubbleReward()
        {
            ...
            MergeManager.PlayerData.ClearDogBubbleGetRewardTime(m_Index);
            MergeManager.PlayerData.SetDogBubbleRewardId(m_Index, 0);
            ...
            GameManager.DataNode.SetData<bool>("DogTreeCanAutoPop", true);
        ...
        {
            DateTime getRewardTime = MergeManager.PlayerData.GetDogBubbleGetRewardTime(m_Index);
            DateTime getRewardRVTime = MergeManager.PlayerData.GetDogBubbleGetRewardRVTime();
            ...
            else if (m_RewardData.EnableAd && GameManager.Ads.CheckRewardedAdIsLoaded() && DateTime.Now >= getRewardRVTime)
            {
                ...
                GameManager.Ads.ShowRewardedAd("DogBubble_" + m_Index.ToString());
                MergeManager.PlayerData.SetDogBubbleGetRewardRVTime(DateTime.Now.AddMinutes(10));
                GameManager.Event.Fire(this, CommonEventArgs.Create(CommonEventType.DogBubbleGetRewardRVTime));
            }
        ...
        public void OnRewardAdEarned(object sender, GameEventArgs e)
        {
            ...
            if (ne.UserData.ToString() != "DogBubble_" + m_Index.ToString())
            {
                RefreshTimerBarIcon();
                return;
            }

            Debug.Log("..........DogBubble OnRewardAdEarned " + ne.EarnedReward.ToString());
