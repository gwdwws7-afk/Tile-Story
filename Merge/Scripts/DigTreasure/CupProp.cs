...
namespace Merge
{
    public sealed class CupProp : Prop
    {
        ...
        public override void OnGeneratedByMerge()
        {
            ...
            GenerateAdditionalOutput(propId);

            if (!MergeManager.PlayerData.GetPropIsUnlock(propId))
            {
                MergeManager.PlayerData.SetPropIsUnlock(propId);
                ...
                mainBoard?.ShowMergeFlyItemSlot(propId, transform.position);
            }
        }
        ...
        private void GenerateAdditionalOutput(int propId)
        {
            List<int> OutputIds = new List<int>();
            if (propId == 60102)
            {
                OutputIds = new List<int>() { 40101 };
            }
            else if(propId == 60103)
            {
                OutputIds = new List<int>() { 40101, 40101 };
            }
            else if (propId == 60104)
            {
                OutputIds = new List<int>() { 40101, 40101, 40102 };
            }
